...
def parse_args():
    ...
    parser.add_argument(
        "--delay", type=float, default=1.0, help="Delay (seconds) between orders"
    )
...
def delivery_report(err, msg):
    if err is not None:
        ...
        logging.info(
            f"✅ Delivered {msg.key()} → {msg.topic()} [{msg.partition()}] @ offset {msg.offset()}"
        )
...
def produce_orders(count: int, delay: float):
    ...
    for i in range(count):
        ...
        producer.produce(
            topic=config.TOPIC, key=order_id, value=payload, on_delivery=delivery_report
        )
